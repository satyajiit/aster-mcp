...
class AsterService : Service() {
    ...
    private suspend fun buildConfig(type: ModeType, intent: Intent): ModeConfig {
        return when (type) {
            ModeType.IPC -> {
                val providedToken = intent.getStringExtra(EXTRA_CONFIG_JSON)
                val token = if (!providedToken.isNullOrEmpty()) {
                    providedToken  // Token pre-generated by approval flow
                } else {
                    ipcMode.generateToken()
                }
                settingsDataStore.saveIpcToken(token)
